    ...
    public void onAction(GuiScreen guiScreen) {
        if (!this.isMultiChunk()) {
            guiScreen.mc.displayGuiScreen(new GuiChat(
                    String.format(WarClaimsConfig.claimSingleCommandFormat, this.startX, this.startZ)
            ));
            return;
        }
        guiScreen.mc.displayGuiScreen(new GuiChat(
                String.format(WarClaimsConfig.claimSelectionCommandFormat, this.startX, this.startZ, this.endX, this.endZ)
        ));
    }
}
